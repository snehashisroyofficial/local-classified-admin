...
const CustomerListTable = () => {
  ...
  const allColumns = [
    columnHelper.display({
      id: "full_name",
      cell: (info) => (
        <div className="max-w-68 mx-auto truncate capitalize">
          <div>
            <div className="flex items-center gap-2">
              <Image
                src={info.row.original.user_avatar || "/images/avatar.png"}
                alt=""
                width={30}
                height={30}
        ...
        <div className="max-w-32 mx-auto truncate">
          <span>
            {info.row.original?.email_verified ? (
              <div className="py-1 px-2 flex items-center gap-1 text-xs bg-green-100 rounded-full text-green-600 w-fit">
                Verified
                <Check size={14} />
              </div>
            ) : (
              <div className="py-1 px-2 flex items-center gap-1 text-xs  bg-red-100 rounded-full text-red-400 w-fit">
                {" "}
                Not Verified
                <X size={14} />
              </div>
    ...
    columnHelper.display({
      ...
      cell: (info) => (
        <div className="relative">
          ...
          {showModalIndex === info.row.index && (
            <div
              ref={popupRef}
              className={`absolute z-20 ${
                showModalIndex + 1 > 4 ? "bottom-10" : "top-10"
              } right-6 bg-white shadow-lg rounded-lg border border-gray-300`}
            >
              <div className="p-2 ">
                <button className="flex items-center justify-items-start gap-2 hover:bg-sidebar-highlight p-2 rounded-md w-full ">
                  <ClipboardClock size={16} />
                  <span className="text-nowrap">Book Appointment</span>
                </button>
                <button
                  disabled
                  className=" flex items-center justify-items-start gap-2 hover:bg-sidebar-highlight p-2 rounded-md w-full"
                >
                  <Users size={16} />
                  <span className="text-nowrap">Assign Member</span>
                </button>
                <button className="flex items-center justify-items-start gap-2 hover:bg-sidebar-highlight p-2 rounded-md w-full">
                  <Info size={16} />
                  <span className="text-nowrap">Customer Details</span>
                </button>
                <button className="flex items-center justify-items-start gap-2 hover:bg-error hover:text-white text-red-600 p-2 rounded-md w-full">
                  <Trash size={16} />
                  <span className="text-nowrap">Delete Customer</span>
                </button>
              </div>
            </div>
          )}
                  ...
                  <tr key={eachHeader.id}>
                    {eachHeader.headers.map((currentHeader) => (
                      <th
                        ...
                        className="p-3 font-semibold text-sm uppercase "
                      >
